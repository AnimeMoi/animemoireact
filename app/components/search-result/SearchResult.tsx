...
const SearchResult: React.FC<SearchResultProps> = ({results}) => {
    return (
        <div
            className="w-[380px] max-h-[444px] flex flex-col flex-grow gap-5 p-[15px] bg-richBlack/80 backdrop-blur-[10px] rounded-3xl border-[1.5px] border-white/20 overflow-y-scroll no-scrollbar overlay-show">
            {results.map((result) => {
                const chapterNumber =
                ...
                return (
                    <div className="flex flex-row gap-[15px]" key={result.id}>
                        <Link href={`/pages/reader/${result.host}?id=${result.id}`}>
